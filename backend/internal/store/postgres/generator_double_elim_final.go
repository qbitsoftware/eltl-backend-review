...
type Generator_Double_Final struct {
	Generator_Base
	phandler *PHandler_Double_Final
}

func CreateGeneratorDoubleFinal(database *Store) *Generator_Double_Final {
	return &Generator_Double_Final{
		Generator_Base: Generator_Base{
			store: database,
		},
		phandler: CreatePHandlerDoubleFinal(database),
	}
}

func (d *Generator_Double_Final) CreateMatches(tournament *model.Tournament, tx *gorm.DB) error {
	fmt.Println("DOUBLE ELIMINATION 2: Creating matches...")
	...
	w, l := utils.CalculateMatchesPerRoundDoubleWeird(len(participants))
	//winners bracket
	for i := 1; i <= len(w); i++ {
		for j := 1; j <= w[i]; j++ {
			var bracket string
...
func (d *Generator_Double_Final) CreateBrackets(tournament *model.Tournament, tx *gorm.DB) error {
	fmt.Println("DOUBLE ELIMINATION 2: Creating brackets...")
...
func (d *Generator_Double_Final) LinkMatches(tournament *model.Tournament, tx *gorm.DB) error {
	fmt.Println("DOUBLE ELIMINATION 2: Linking matches...")
...
func (d *Generator_Double_Final) AssignPlayers(tournament *model.Tournament, tx *gorm.DB) error {
	fmt.Println("DOUBLE ELIMINATION 2: Assigning players...")
...
func (d *Generator_Double_Final) LinkWinners(tournament *model.Tournament, tx *gorm.DB) error {
	...
	for _, match := range winnerMatches {
		...
		maxLRounds, err := d.store.Match().GetLoserMaxRounds(tournament.ID, tx)
		if err != nil {
			return err
		}
		if maxLRounds == nextLoserMatchRound {
			//move that to the 3-4 place match
			weirdStuff = "winner"
			nextLoserMatchRound = match.CurrentRound + 1
			nextLoserMatchIdentifier = 2
		}

		//update the match (find the right match id-s)
		if err := tx.Model(&model.Match{}).Where("current_round = ? AND identifier = ? AND tournament_id = ? AND type = ?", nextLoserMatchRound, nextLoserMatchIdentifier, match.TournamentID, weirdStuff).First(&loserMatch).Error; err != nil {
			return err
		}
...
func (d *Generator_Double_Final) LinkLosers(tournament *model.Tournament, tx *gorm.DB) error {
	...
	for _, match := range loserMatches {
		...
		if (match.CurrentRound+1)%2 == 0 && matchCurrentBracket[0] > 8 {
