...
type PHandler_Double_Final struct {
	store *Store
}

func CreatePHandlerDoubleFinal(database *Store) *PHandler_Double_Final {
	return &PHandler_Double_Final{
		store: database,
	}
}

func (p *PHandler_Double_Final) MovePlayers(match *model.Match, tx *gorm.DB) error {
	fmt.Println("DOUBLE ELIMINATION 2: Moving players...")
...
func (p *PHandler_Double_Final) CheckByeBye(match model.Match, tx *gorm.DB) error {
...
func (p *PHandler_Double_Final) NextMatch(match model.Match, winner, loser uint, tx *gorm.DB) error {
